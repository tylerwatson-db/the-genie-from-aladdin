...
const tablesSchema = z.object({
  segmentPosition: z.string().min(1),
  openSliding: z.string().min(1),
  actionRecommendations: z.string().min(1),
...
await createApp({
  ...
  if (!appConfig.masEndpointName && !appConfig.genieSpaceId) {
    console.warn(
      '[boot] No ask_data backend configured. Set MAS_ENDPOINT_NAME or GENIE_SPACE_ID.',
    );
  } else {
    console.log(
      `[boot] ask_data backend: ${appConfig.masEndpointName ? 'MAS' : 'Genie'}`,
    );
  }
...
void (async () => {
  ...
  if (agentExperimentId) {
    // The app runs as its assigned service principal. DATABRICKS_AUTH_TYPE is
    // pinned to oauth-m2m in app.yaml so mlflow-tracing's bundled SDK does not
    // combine the injected OAuth credentials with a PAT from ~/.databrickscfg.
    ...
    try {
      ...
      console.warn('[boot] could not resolve MLflow exporter host from the app client:', (e as Error).message);
    }
